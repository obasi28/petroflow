...
| 2 | 2026-02-24 | Well metadata import rejects title-case enum values (e.g., `Oil`, `Shut In`) and skips valid rows. | high | done | Added enum normalization for well import payloads (case-insensitive + alias mapping) before validation, so common CSV labels map to canonical backend values. | pending |
